...
def valid_reshape_args(draw):
    ...
    if np.issubdtype(input_dtype, np.integer):
        ...
        min_bfloat16 = float(ml_dtypes.finfo(reshape_types.get(inputs_attributes["ONNXRuntime_Provider"])["BFLOAT16"]).min)
        max_bfloat16 = float(ml_dtypes.finfo(reshape_types.get(inputs_attributes["ONNXRuntime_Provider"])["BFLOAT16"]).max)
...
def calculate_flatindex(outer_loop_value, inner_loop_value, shape, coords):
    """
    Function to calculate flat index from a tensor
    """
    result = 0
    for i in range(outer_loop_value):
        prod = 1
        for j in range(i+1, inner_loop_value):
            prod *= shape[j]
        result += coords[i] * prod
    return result


def calculate_remainder_x(flatindex, coords, shape, index):
    """
    Function to calculate remainder of flat index from coordinates and shape 
    """
    remainderx = calculate_flatindex (index, len(shape), shape, coords)
    return flatindex - remainderx


def calculate_xcoords_index (y_shape, y_coords, x_shape, x_coords, x_index):
    """
    Function to calculate x coordinate from coordinates, shape and index
    """
    flatindex = calculate_flatindex (len(y_shape), len(y_shape), y_shape, y_coords)
    remainderx = calculate_remainder_x (flatindex, x_coords, x_shape, x_index)
    denominator = 1
    for i in range (x_index+1, len(x_shape)):
        denominator *= x_shape[i]
    xcoord = math.floor (remainderx / denominator)
    return xcoord

def calculate_xcoords(y_shape, y_coords, x_shape, x_coords):
    """
    Function to calculate all x coordinates
    """
    for i in range (len(x_shape)):
        xcoord = calculate_xcoords_index (y_shape, y_coords, x_shape, x_coords, i)
        x_coords[i] = xcoord
    return x_coords

def check_coords_value (x,y):
    result = []
    it = np.nditer(y, flags=['multi_index', 'refs_ok'])
    for y_value in it:
        y_coords = it.multi_index
        x_coords = [0] * len(x.shape)
        x_coords = calculate_xcoords (list(y.shape), y_coords, list(x.shape), x_coords)
        x_value = x[tuple(x_coords)]
...
def check_constraints(x, s, allowzero, y, y_shape):
    ...
    """
    assert list(y.shape) == y_shape, f"Output shape {y.shape} does not match expected shape {y_shape}"

    assert check_coords_value (x,y), "Values in output tensor do not match expected values from input tensor"
